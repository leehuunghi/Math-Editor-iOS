...
- (void) initialize
{
    // Add gesture recognizer to let the user enter editing mode.
    ...
    [self addGestureRecognizer:self.panGestureRecognize];
...
- (CGPoint) caretPoint
{
    return _caretView.frame.origin;
}
...
- (void)pinch:(UIPinchGestureRecognizer *)pinchGesture {
    ...
    UIView *pinchView = pinchGesture.view;
    CGRect bounds = pinchView.bounds;
    CGPoint pinchCenter = [pinchGesture locationInView:pinchView];
...
- (void) enablePan:(BOOL) enabled {
    self.panGestureRecognize.enabled = enabled;
}
- (void) enablePinch:(BOOL) enabled {
